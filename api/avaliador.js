...
export default async function handler(req, res) {
  // Garantir cabeçalhos de resposta JSON
  ...
  try {
    ...
    // 2. Ver Provas do Avaliador
    if (req.method === 'GET' && action === 'listarProvas') {
      const { nick } = req.query;
      const nickAvaliador = nick || '';
      const provas = await sql`
        SELECT * FROM provas WHERE avaliador_nick = ${nickAvaliador} ORDER BY criado_em DESC
      `;
    ...
    console.error('Erro de execução Serverless:', error);
